  ...
  var treeDir = {
    ...
    manageEvents: function(s, e, a) {
      if (a.treeEvents) {
        ...
        treeDir.tree.on('select_node.jstree',function(a,b){
          s.vm.nodeSelected(s,b);
        });
      }
    },
	...
	// users can define 'core'
        ...
        if (a.treeData == 'html') {
          ...
          config.core.data = {
            'url': a.treeAjax,
            'data': function(node) {
              console.log("tree ajax",node);
              return {
                'id': node.id != '#' ? node.id : 1,
                'apiType': (node.li_attr&&node.li_attr.apiType?node.li_attr.apiType:null),
                'secondFilter': (node.li_attr&&node.li_attr.secondFilter?node.li_attr.secondFilter:null)
              };
            }
          };
          treeDir.init(s, e, a, config);
